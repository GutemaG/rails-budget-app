...
class CostsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create cost' do
    assert_difference('Cost.count') do
      post costs_url, params: { cost: { amount: @cost.amount, author_id: @cost.author_id, name: @cost.name } }
    end
  ...
  test 'should update cost' do
    patch cost_url(@cost), params: { cost: { amount: @cost.amount, author_id: @cost.author_id, name: @cost.name } }
